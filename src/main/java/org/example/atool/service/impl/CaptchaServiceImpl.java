...
public class CaptchaServiceImpl implements CaptchaService {
    ...
    public void send(String type, String target) {
        ...
        String resendCheckKey = StrUtil.format("resend:{}",target);
        if(redisClient.has(resendCheckKey)){
            Throw.BizExp(StrUtil.format("请勿{}分钟内重复获取",prop.getResendInAs(TimeUnit.MINUTES)));
        }
        captchaSender.send(type, code, target);

        redisClient.set(resendCheckKey,target, prop.getResendIn(), prop.getResendUnit());

        String expireCheckKey = StrUtil.format("captcha:{}:{}", type, target);
        redisClient.set(expireCheckKey,code ,prop.getExpire(),prop.getExpireUnit());
    }
